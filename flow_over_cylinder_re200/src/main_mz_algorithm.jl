...
function svd_low_rank_proj_observables(X1, X, r)
    U, S, V = svd(X1);
...
function obtain_Ω_modes(Ω)
    ty = typeof(Ω[1,1,1]);
    Ω_modes = zeros(ty, size(Ω)[1], size(Ω)[2], size(Ω)[3]);
    λ = complex(zeros(size(Ω)[1], size(Ω)[3]));
    for n_m in 2 : size(Ω)[1]
        λ[n_m, :], Ω_ = eigen(Ω[n_m, :, :]); #Eigen modes of M
        Ω_modes[n_m,:,:] = real.(Ω_)
    end
    return Ω_modes, λ
end

function obtain_lifted_Ω_modes(Ω_m, Ur)
    ty = typeof(Ω_m[1,1,1]);
    Ωm_lift = zeros(ty, size(Ω_m)[1], size(Ur)[1], size(Ω_m)[3]);
    for n_m in 2 : size(Ω_m)[1]
        Ωm_lift[n_m,:,:] = Ur * Ω_m[n_m,:,:];
    end
    return Ωm_lift
end
