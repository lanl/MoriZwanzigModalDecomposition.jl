...
function animate_field(X, method, out_path)
    file_out = out_path
    T = size(X, 2);
    anim = @animate for t ∈ 1:T
        println("time step animation = ", t)
        plt = plot_field(X, t, method)
    end
    mov(anim, file_out, fps = ceil(Int, T/10))
end
...
function plot_hodmd_modes(Φ, m, d)
    gsy = 600; gsx = round(Int, 2.25*gsy)
    gr(size=(gsx,gsy));
    nx = 199; ny = 449;
    Φ = reshape(Φ, (199, 449, d*r));
    r_2 = floor(Int, n_ks*r/2)
    ...
    mode_n = ceil(Int, m/2)
    title!(L"\textrm{HODMD ~ mode ~ } \Phi^{hodmd}_{%$mode_n}", titlefont=22)
    xlabel!(L"\textbf{x}", xtickfontsize=12, xguidefontsize=20)
    ylabel!(L"\textbf{y}", ytickfontsize=12, yguidefontsize=20)
    # savefig(plt, "./figures/mzmd_Markovian_mode$(m)_nk$(n_ks)_r$(r).png")
    return plt
...
function plot_mzmd_modes(Φ, m, n_ks)
    gsy = 600; gsx = round(Int, 2.25*gsy)
    gr(size=(gsx,gsy));
    nx = 199; ny = 449;
    Φ = reshape(Φ, (199, 449, n_ks*r));
    ...
    if n_ks>1
        ...
        title!(L"\textrm{DMD ~ mode ~ } \Phi^{dmd}_{%$mode_n}", titlefont=22)
    end
    xlabel!(L"\textbf{x}", xtickfontsize=12, xguidefontsize=20)
    ylabel!(L"\textbf{y}", ytickfontsize=12, yguidefontsize=20)
    # savefig(plt, "./figures/mzmd_Markovian_mode$(m)_nk$(n_ks)_r$(r).png")
...
function plot_c_spectrum(Λc)
    ...
    plt = scatter(Λc, ms = 4.0, color = "black", legend=false, grid = true, framestyle = :box)
